...
function renderDrawing(currentUserDrawings){
 ...
 function addElementListeners(object, eventType) {
     Object.keys(object).forEach(key => {
         document.getElementById(key).addEventListener(eventType, event => object[key](event));
     });
 };
...
function verifyUser(event){
    ...
    } else if(allUsers.filter(user => user.username == usernameInput).length > 0){
...
function deleteDrawing(event){
    ...
    let drawing = currentUserDrawings.findIndex(drawing => drawing.id == drawingId)
